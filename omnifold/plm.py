...
from omnifold.inputs import IS_PROTEIN
from omnifold.tensor_typing import Float, Int, typecheck
from omnifold.utils.data_utils import join
...
class ESMWrapper(Module):
    ...
    def forward(
        ...
        """
        device, seq_len, repr_layer = aa_ids.device, aa_ids.shape[-1], self.repr_layer
...
class ProstT5Wrapper(Module):
    ...
    def forward(
        ...
        :return: The PLM embeddings for the input sequences.
        """
        device, seq_len = aa_ids.device, aa_ids.shape[-1]
