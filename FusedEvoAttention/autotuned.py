...
def _attn_fwd(
    ...
    block_index_q = tl.program_id(0).to(tl.int64)

    index_batch_msa_head = tl.program_id(1).to(tl.int64)
...
def _attn_bwd_dq(
    ...
    index_block_kv = tl.program_id(0).to(tl.int64)
    ...
    for blk_idx in range(num_steps):
        ...
        tl.atomic_add(d_pair_bias_block_ptr, dS_block, mask=(offs_q[:, None] < SEQ_LEN) & ((blk_idx * BLOCK_SIZE_KV + offs_kv)[None, :] < SEQ_LEN), sem="relaxed") 
...
class EvoformerAttention(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, res_mask, pair_bias):
        ...
        _attn_fwd[grid](
            ...
            BATCH_SIZE=BATCH_SIZE, HEAD=HEAD, N_SEQ=N_SEQ, SEQ_LEN=SEQ_LEN, DIM=DIM, BLOCK_DIM=BLOCK_DIM,
            **extra_kern_args,
            # H200; seq-len=384
            BLOCK_SIZE_Q=16, BLOCK_SIZE_KV=64, num_stages=3, num_warps=4
        )
    ...
    def backward(ctx, dO):
        ...
        _attn_bwd_preprocess[preprocess_grid](
            ...
            BLOCK_DIM=BLOCK_DIM,
            # H200; seq-len=384
            BLOCK_SIZE_Q=32, num_stages=2, num_warps=4
        )
        ...
        _attn_bwd_dk_dv[bwd_dk_dv_grid](
            ...
            DIM=ctx.DIM,
            # H200; seq-len=384
            BLOCK_SIZE_Q=64, BLOCK_SIZE_KV=16, num_stages=3, num_warps=4, 
        )
...
if __name__ == "__main__":
    # Run autotune for inputs
    BATCH, H, HEAD_DIM, N_SEQ, N_CTX = 4, 16, 64, 1, 384 # attentionpairbias
    # BATCH, H, HEAD_DIM, N_SEQ, N_CTX = 1, 4, 32, 384, 384 # triangle attention inputs
