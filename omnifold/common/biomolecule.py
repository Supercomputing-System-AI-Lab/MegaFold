...
from omnifold.life import DNA_NUCLEOTIDES, HUMAN_AMINO_ACIDS, RNA_NUCLEOTIDES
from omnifold.tensor_typing import IntType, typecheck
from omnifold.utils.data_utils import (
    deep_merge_dicts,
    get_sorted_tuple_indices,
    is_polymer,
)
from omnifold.utils.utils import exists, not_exists, np_mode
...
class Biomolecule:
    ...
    bonds: List[mmcif_parsing.Bond] | None  # [num_bonds]
    ...
    unique_res_atom_names: List[
        Tuple[List[List[str]], str, int]
    ] | None  # [num_res, num_pseudoresidues, num_atoms]
    ...
    def subset_chains(self, subset_chain_ids: List[str]) -> "Biomolecule":
        ...
        chain_id_mapping = {chain_id: n for n, chain_id in enumerate(dict.fromkeys(self.chain_id))}
        assert all(
    ...
    def crop_chains_with_masks(
        self,
        chain_ids_and_lengths: List[Tuple[str, int]],
        crop_masks: List[np.ndarray],
        cropping_type: Literal["contiguous", "spatial"],
    ) -> "Biomolecule":
        ...
        assert all(
            chain_id == c_id
            for chain_id, (c_id, _) in zip(dict.fromkeys(self.chain_id), chain_ids_and_lengths)
        ), "The cropping chain ID ordering must match the Biomolecule chain ID ordering."
        assert all(
            len(crop_mask) == chain_len
            for crop_mask, (_, chain_len) in zip(crop_masks, chain_ids_and_lengths)
        ), "The cropping mask length must match the cropping chain length."
        assert not all(
            not crop_mask.any() for crop_mask in crop_masks
        ), "Not all tokens can be cropped out of a Biomolecule."

        # NOTE: Protenix reports excluding ions when contiguously cropping
        # and in general keeping atomized token atoms unsegmented as follows:

        is_ligand_token = self.chemtype == 3

        chem_comp_table = {comp.id: comp for comp in self.chem_comp_table}
        comp_details = [chem_comp_table[chemid] for chemid in self.chemid]
        is_modified_polymer_token = np.array(
            [
                get_resname_and_modified_status(
                    chem_comp, get_residue_constants(res_chem_index=chemtype)
                )[1]
                for (chem_comp, chemtype) in zip(comp_details, self.chemtype)
            ]
        )

        for chain_i, (chain_id, chain_len) in enumerate(chain_ids_and_lengths):
            is_ligand_molecule_token = is_ligand_token[self.chain_id == chain_id] * (chain_len > 1)
            is_ion_chain = is_ligand_token[self.chain_id == chain_id].any() and chain_len == 1

            is_atomized_molecule_token = (
                is_ligand_molecule_token | is_modified_polymer_token[self.chain_id == chain_id]
            )

            if is_ion_chain and cropping_type == "contiguous":
                crop_masks[chain_i] = np.zeros(chain_len, dtype=bool)
            elif is_atomized_molecule_token.any():
                is_segmented_atomized_molecule_token = np.zeros(chain_len, dtype=bool)
                chain_residue_index = self.residue_index[self.chain_id == chain_id]

                for token_i in np.where(is_atomized_molecule_token)[0]:
                    token_residue_index = chain_residue_index[token_i]
                    is_current_residue_token = chain_residue_index == token_residue_index
                    is_segmented_atomized_molecule = not crop_masks[chain_i][
                        is_current_residue_token
                    ].all()
                    is_segmented_atomized_molecule_token[
                        is_current_residue_token * is_segmented_atomized_molecule
                    ] = True

                crop_masks[chain_i][is_segmented_atomized_molecule_token] = False

        # collect metadata for each chain

        unique_chain_ids = list(dict.fromkeys(self.chain_id))
        chains_to_remove = {
    ...
    def contiguous_crop(self, n_res: int = 384) -> Tuple["Biomolecule", List[np.ndarray]]:
        ...
        # reorder the cropping masks and their metadata according to the input chain ordering
        biomol_chain_ids = list(
            dict.fromkeys(self.chain_id.tolist())
        ...
        chain_ids_and_lengths = [chain_ids_and_lengths[idx] for idx in chain_id_sorted_indices]
        crop_masks = [crop_masks[idx] for idx in chain_id_sorted_indices]

        return (
            self.crop_chains_with_masks(
                chain_ids_and_lengths, crop_masks, cropping_type="contiguous"
            ),
            crop_masks,
        )
    ...
    def spatial_crop(
    ...
    ) -> Tuple["Biomolecule", List[np.ndarray]]:
        ...
        token_num_chains = len(dict.fromkeys(self.chain_id))
        ...
        if token_center_atom_indices.size == 0:
            raise ValueError(
                f"No atoms found in the specified chain(s) ({chain_1}, {chain_2}) after spatial cropping."
            )
        ...
        return (
            self.crop_chains_with_masks(
                chain_ids_and_lengths, crop_masks, cropping_type="spatial"
            ),
            crop_masks,
        )
...
@typecheck
def extract_ligand_atom_element(atom_name: str) -> str:
    """Extracts the element symbol from the atom name in an mmCIF file.

    Handles both standard atom names and special cases like 'O5''.
    """
    # Regex to capture the element symbol (assuming it's the first part of the atom name)
    match = re.match(r"([A-Z]+[a-z]*)", atom_name)
    if match:
        return match.group(1)
    return "ATM"  # Fallback if no match is found
...
def get_ligand_atom_name(atom_name: str, atom_types_set: Set[str]) -> str:
    ...
    # Ensure the first character is uppercase for inputs such as 'na' or 'k'
    upper_atom_name = (
        atom_name[0].upper() if len(atom_name) == 1 else atom_name[0].upper() + atom_name[1:]
    )

    # Handle special cases like 'CH3' and 'NH2'
    upper_atom_name = (
        upper_atom_name[0]
        if len(upper_atom_name) > 1
        and upper_atom_name[0] != "R"
        and upper_atom_name[1].upper() == "H"
        else upper_atom_name
    )

    # Extract the element symbol from the atom name
    element = extract_ligand_atom_element(upper_atom_name)

    # Use uppercase for consistent matching
    element = element.upper()

    # Remove hydrogen prefix if necessary
    if element not in atom_types_set and element.startswith("H") and len(element) > 1:
        element = element[1:]

    # Remove uncommon suffixes such as 'G' in 'PG'
    if element not in atom_types_set and element[0] in atom_types_set:
        element = element[0]

    # Map element to atom_types or fallback to 'ATM'
    return element if element in atom_types_set else "ATM"
...
def get_unique_res_atom_names(
    ...
    for chain in mmcif_object.structure:
        ...
        for res, res_chem_comp in zip(chain, chain_chem_comp):
            ...
            _, is_modified_polymer_residue = get_resname_and_modified_status(
                res_chem_comp, residue_constants
            )
            if is_polymer_residue and not is_modified_polymer_residue:
                ...
                atoms_to_append = [
                    [atom.name for _ in range(lig_res_constants.atom_type_num)] for atom in res
                ]
...
@typecheck
def get_resname_and_modified_status(
    comp_details: mmcif_parsing.ChemComp, residue_constants: ModuleType
) -> Tuple[str, bool]:
    """Get the residue name and modified polymer residue status for a given `ChemComp` object.

    :param comp_details: The `ChemComp` object for the residue.
    :param residue_constants: The residue constants module for the residue type.
    :return: The (mapped) residue name and its modified polymer residue status.
    """
    is_polymer_residue = is_polymer(comp_details.type)
    is_peptide_residue = "peptide" in comp_details.type.lower()

    is_rna_residue = "rna" in comp_details.type.lower()
    is_dna_residue = "dna" in comp_details.type.lower()
    is_nucleotide_residue = is_rna_residue or is_dna_residue

    res_shortname = residue_constants.restype_3to1.get(comp_details.id, "X")

    # Detect modified polymer residues.
    is_modified_polymer_residue = (
        is_polymer_residue
        and res_shortname == "X"
        and (
            (is_peptide_residue and comp_details.id in PROTEIN_LETTERS_3TO1_EXTENDED)
            or (is_nucleotide_residue and comp_details.id in NUCLEIC_LETTERS_3TO1_EXTENDED)
        )
    )
    if is_modified_polymer_residue:
        # Try remapping the residue name to a known polymer residue type.
        remapped_res_shortname = (
            PROTEIN_LETTERS_3TO1_EXTENDED[comp_details.id]
            if is_peptide_residue
            else NUCLEIC_LETTERS_3TO1_EXTENDED[comp_details.id]
        )
        if len(remapped_res_shortname) == 1:
            if is_peptide_residue:
                remapped_resname = PROTEIN_LETTERS_1TO3[remapped_res_shortname]
            elif is_rna_residue:
                remapped_resname = RNA_LETTERS_1TO3[remapped_res_shortname]
            elif is_dna_residue:
                remapped_resname = DNA_LETTERS_1TO3[remapped_res_shortname]
            res_shortname = residue_constants.restype_3to1.get(remapped_resname, "X")

    return res_shortname, is_modified_polymer_residue
...
def _from_mmcif_object(
    ...
    raise_on_missing_polymer_center_atoms: bool = False,
) -> Biomolecule:
    ...
    :param raise_on_missing_polymer_center_atoms: If True, then raise an error if any
        polymer residue is missing its representative atom (e.g., CA for peptides).
    ...
    if isinstance(structure, Model):
        ...
        if len(models) != 1:
            raise ValueError(
                f"Only single model mmCIFs are supported. Found {len(models)} models."
            )
        model = models[0]
    ...
    ligand_residue_constants = get_residue_constants("ligand")
    ...
    for chain in model:
        ...
        for res_index, res in enumerate(chain):
            if res.id[2] != " ":
                raise ValueError(
                    f"mmCIF {mmcif_object.file_id} contains an insertion code at chain {chain.id} and"
            ...
            res_shortname, is_modified_polymer_residue = get_resname_and_modified_status(
                res_chem_comp_details, residue_constants
            )

            restype_idx = residue_constants.restype_order.get(
                res_shortname, residue_constants.restype_num
            )
            is_nonstandard_polymer_residue = (
                is_polymer_residue
                and not is_modified_polymer_residue
                and restype_idx == residue_constants.restype_num
            )
            ...
            if residize_polymer or residize_non_polymer:
                ...
                for atom in res:
                    # NOTE: Protenix reports using the first occupancy of each atom as follows, not the (default) highest occupancy:
                    if atom.is_disordered():
                        try:
                            first_altloc = sorted(atom.child_dict.keys())[0]
                            atom.disordered_select(first_altloc)
                        except Exception as e:
                            print(
                                f"Failed to select the first altloc for polymer atom {atom.name} in residue {res.get_resname()} at chain {chain.id} and residue index {res.id[1]} (residue number {res_index + 1}) for mmCIF {mmcif_object.file_id} due to: {e}."
                                "Reverting to the default behavior of selecting the highest occupancy."
                            )
                    if is_polymer_residue and atom.name not in residue_constants.atom_types_set:
                        continue
                    elif (
                        is_nonstandard_polymer_residue
                        and atom.name not in residue_constants.unk_restype_compact_atom_names_set
                    ):
                        continue
                    elif (
                        is_peptide_residue
                ...
                if (
                    is_polymer_residue
                    and raise_on_missing_polymer_center_atoms
                    and not pos[residue_constants.res_rep_atom_index].any()
                ):
                    raise ValueError(
                        f"Polymer residue {res.get_resname()} in chain {chain.id} at residue index {res.id[1]} (residue number {res_index + 1}) for mmCIF {mmcif_object.file_id} is missing its representative atom {residue_constants.atom_types[residue_constants.res_rep_atom_index]}. "
                        "This can cause downstream parsing errors. To ignore this error, set `raise_on_missing_polymer_center_atoms=False`."
                    )
                if is_peptide_residue:
                ...
                for atom in res:
                    ...
                    if atom.is_disordered():
                        try:
                            ...
                            print(
                                f"Failed to select the first altloc for 'pseudoresidue' atom {atom.name} in residue {res.get_resname()} at chain {chain.id} and residue index {res.id[1]} (residue number {res_index + 1}) for mmCIF {mmcif_object.file_id} due to: {e}."
                                "Reverting to the default behavior of selecting the highest occupancy."
                            )
                    # NOTE: This code assumes water residues have previously been filtered out.
                    pos = np.zeros((ligand_residue_constants.atom_type_num, 3))
                    mask = np.zeros((ligand_residue_constants.atom_type_num,))
                    res_b_factors = np.zeros((ligand_residue_constants.atom_type_num,))

                    atom_name = atom.name
                    if atom_name not in ligand_residue_constants.atom_types_set:
                        atom_name = get_ligand_atom_name(
                            atom_name, ligand_residue_constants.atom_types_set
                        )
                    if atom_name not in ligand_residue_constants.atom_types_set:
                        atom_name = "ATM"
                    atom_names.append(atom_name)

                    pos[ligand_residue_constants.atom_order[atom_name]] = atom.coord
                    mask[ligand_residue_constants.atom_order[atom_name]] = 1.0
                    res_b_factors[ligand_residue_constants.atom_order[atom_name]] = atom.bfactor

                    restype.append(restype_idx)
                    chemid.append(res_chem_comp_details.id)

                    # NOTE: We retain the original chemical type for modified polymer residues.
                    chemtype.append(residue_constants.chemtype_num)
...
def to_mmcif(
    ...
    insert_omnifold_mmcif_metadata: bool = True,
    return_only_atom_site_records: bool = False,
    unique_res_atom_names: List[Tuple[List[List[str]], str, int]] | None = None,
    model_number: int = 1,
    confidence_model_number: int | None = None,
) -> str:
    ...
    :param insert_omnifold_mmcif_metadata: If True, insert metadata fields
        referencing OmniFold in the output mmCIF file.
    :param return_only_atom_site_records: If True, only return the _atom_site loop records.
    :param unique_res_atom_names: A dictionary mapping each author chain ID to a list of lists
        ...
        This is used to group "pseudoresidues" (e.g., ligand atoms) by parent residue.
    :param model_number: The model number to be listed in the mmCIF.
    :param confidence_model_number: The confidence model number to be listed in the mmCIF.
    ...
    # Via unique chain-residue indexing, ensure that each atomized residue
    # is represented by only a single atom for all logic except atom site parsing.
    # NOTE: Since the structure may start with any chain ID (not necessarily `A`),
    # we must maintain the consecutive order of unique chain IDs.
    chain_index_ordering = {
        c_id: idx for idx, c_id in enumerate(dict.fromkeys(biomol.chain_index))
    }
    chain_index_ordered = np.vectorize(chain_index_ordering.get)(biomol.chain_index)
    _, unique_indices = np.unique(
        np.array(list(zip(chain_index_ordered, biomol.residue_index))), axis=0, return_index=True
    )
    ...
    unique_comp_details = [chem_comp_table[chemid] for chemid in unique_chemid]
    ...
    for i in range(unique_restype.shape[0]):
        ...
        atom_residue_constants = residue_constants = get_residue_constants(
            res_chem_index=res_chemindex
        )
        mapped_unique_res_shortname, is_modified_polymer_residue = get_resname_and_modified_status(
            unique_comp_details[i], residue_constants
        )
        mapped_unique_restype = residue_constants.restype_order.get(
            mapped_unique_res_shortname, residue_constants.restype_num
        )
        if is_modified_polymer_residue:
            atom_residue_constants = get_residue_constants("ligand")
        ...
        if (mapped_unique_restype - residue_constants.min_restype_num) <= len(
            residue_constants.restypes
        ):
            res_atom_names = (
                unique_res_atom_names[i]
                if exists(unique_res_atom_names)
                else [atom_residue_constants.atom_types for _ in range(len(res_atom_positions))]
            )
        ...
        for atom_name_, pos_, mask_, b_factor_ in zip(
            ...
            for atom_name, pos, mask, b_factor in zip(atom_name_, pos_, mask_, b_factor_):
                ...
                atom_index_suffix = "" if is_polymer_residue else unique_atom_indices[atom_name]
                unique_atom_name = (
                    atom_name
                    if exists(unique_res_atom_names)
                    else f"{atom_name}{atom_index_suffix}"
                )
                ...
                mmcif_dict["_atom_site.pdbx_PDB_model_num"].append(str(model_number))
    ...
    init_metadata_dict = (
        ...
        if insert_omnifold_mmcif_metadata
    ...
    metadata_dict = mmcif_metadata.add_metadata_to_mmcif(
        mmcif_dict,
        insert_omnifold_mmcif_metadata=insert_omnifold_mmcif_metadata,
        model_number=confidence_model_number,
    )
    init_metadata_dict.update(metadata_dict)
    init_metadata_dict.update(mmcif_dict)

    if return_only_atom_site_records:
        init_metadata_dict = {
            key: value
            for key, value in init_metadata_dict.items()
            if key.startswith("data_") or key.startswith("_atom_site.")
        }
...
def to_inference_mmcif(
    ...
    """Converts an `OmniFoldInput`-derived `Biomolecule` instance to an mmCIF string. Clone of
    ...
    :param insert_omnifold_mmcif_metadata: If True, insert metadata fields
        referencing OmniFold in the output mmCIF file.
    :param return_only_atom_site_records: If True, return only the atom site records.
    :param unique_res_atom_names: A dictionary mapping each author chain ID to a list of lists
        of lists of atom names for each "pseudoresidue" of each unique residue. If None, the
        atom names are assumed to be the same for all residues of the same chemical type (e.g., RNA).
        This is used to group "pseudoresidues" (e.g., ligand atoms) by parent residue.
    ...
    # NOTE: For inference only, we replace the input SMILES strings with `LIG`
    # to avoid running into formatting issues with the mmCIF file format.
    chemid = np.where(np.isin(chemtype, [3, 4]), "LIG", chemid)
    ...
    biomol_chem_comp_table = biomol.chem_comp_table | {
        mmcif_parsing.ChemComp(
            id="LIG",
            name="LIG",
            formula="LIG",
    ...
    chem_comp_table = {comp.id: comp for comp in biomol_chem_comp_table}
    unique_comp_details = [chem_comp_table[chemid] for chemid in unique_chemid]
    ...
    if biomol_chem_comp_table:
        for chem_comp in biomol_chem_comp_table:
            mmcif_dict["_chem_comp.id"].append(chem_comp.id)
            mmcif_dict["_chem_comp.formula"].append(chem_comp.formula)
            mmcif_dict["_chem_comp.formula_weight"].append(chem_comp.formula_weight)
            mmcif_dict["_chem_comp.mon_nstd_flag"].append(chem_comp.mon_nstd_flag)
    ...
    for chain_id, (res_ids, chemids, chemindices) in _get_chain_seq(
        ...
        for res_id, res_chemid, res_chemindex in zip(res_ids, chemids, chemindices):
            ...
            # NOTE: When ligand, metal, and non-canonical support is better, this can be made more elegant.
            if res_chemid != residue_constants.unk_restype and res_chemid not in chem_comp_ids:
    ...
    for chain_id, (res_ids, chemids, chemindices) in _get_chain_seq(
        ...
        for res_id, res_chemid, res_chemindex in zip(res_ids, chemids, chemindices):
            ...
            if res_chemid == residue_constants.unk_restype and res_chemid not in chem_comp_ids:
                ...
                mmcif_dict["_chem_comp.formula"].append(
                    "?"
                )  # NOTE: This can be updated with a more accurate value
                mmcif_dict["_chem_comp.formula_weight"].append(
                    "?"
                )  # NOTE: This can be updated with a more accurate value
                mmcif_dict["_chem_comp.mon_nstd_flag"].append("no")
    ...
    for i in range(unique_restype.shape[0]):
        ...
        res_chemindex = unique_chemtype[i]
        is_polymer_residue = (res_chemindex < 3).item()
        is_metal_ion = (res_chemindex == 4).item()
        res_name_3 = unique_chemid[i]
        atom_residue_constants = residue_constants = get_residue_constants(
            res_chem_index=res_chemindex
        )
        ...
        for atom_name_, pos_, mask_, b_factor_ in zip(
            ...
            for atom_name, pos, mask, b_factor in zip(atom_name_, pos_, mask_, b_factor_):
                ...
                atom_index_suffix = (
                    ""
                    if (is_polymer_residue or is_metal_ion) and not is_modified_polymer_residue
                    else unique_atom_indices[atom_name]
                )
                unique_atom_name = (
                    atom_name
                    if exists(unique_res_atom_names)
