...
from omnifold.data.data_pipeline import GAP_ID
...
def _match_rows_by_sequence_similarity(
    this_species_msa_dfs: List[pl.DataFrame | None],
) -> List[np.ndarray]:
    ...
    :param this_species_msa_dfs: A list of optional DataFrames containing MSA features for sequences
...
def pad_features(feature: np.ndarray, feature_name: str) -> np.ndarray:
    ...
    """
    assert feature.dtype != np.dtype(np.bytes_), "Feature dtype is bytes, cannot pad."
    if feature_name in (
        "msa_all_seq",
...
def concatenate_paired_and_unpaired_features(
    ...
    """
    if not_exists(max_msas_per_chain):
    ...
    for feature_name in MSA_FEATURES:
        if feature_name in chains:
            is_filtered_sequence_mask = chains["is_unfiltered"] == False  # noqa: E712

            feat = chains[feature_name]
            feat_all_seq = chains[feature_name + "_all_seq"]

            unfiltered_paired_feat = feat_all_seq[:max_paired_msa_per_chain]
            filtered_feat = feat[is_filtered_sequence_mask]
            unfiltered_feat = feat[~is_filtered_sequence_mask]

            chain_merged = np.concatenate(
                [unfiltered_paired_feat, filtered_feat, unfiltered_feat], axis=0
            )
            chains[feature_name + "_all_seq"] = chain_merged[:max_msas_per_chain]

            if verbose and feature_name == "msa":
                if len(unfiltered_paired_feat) > 1:
                    logger.info(
                        f"Successfully paired {len(unfiltered_paired_feat)} unfiltered sequences, "
                        f"with {len(filtered_feat)} filtered sequences and "
                        f"{len(unfiltered_feat)} unfiltered sequences appended thereafter."
                    )
                else:
                    logger.info(
                        f"Did not pair any unfiltered sequences ({len(unfiltered_paired_feat)}), with {len(filtered_feat)} filtered sequences "
                        f"and {len(unfiltered_feat)} unfiltered sequences appended thereafter."
                    )
