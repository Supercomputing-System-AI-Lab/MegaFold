...
from omnifold.nlm import NLMEmbedding
import os 
if os.environ.get("AF3_OPTIMIZATIONS_MODE") == "baseline": 
    ...
    from omnifold.omnifold_layernormlinear import OmniFold
else:
    from omnifold.megafold import OmniFold
from omnifold.plm import PLMEmbedding
from omnifold.tensor_typing import typecheck
from omnifold.trainer import Dataset, Fabric, LRScheduler, Optimizer, Trainer
...
# base pydantic classes for constructing omnifold and trainer from config files
...
class OmniFoldConfig(BaseModelWithExtra):
    """Pydantic model for the OmniFold configuration."""
    ...
    @classmethod
    @typecheck
    def from_yaml_file(cls, path: str | Path, dotpath: str | List[str] = []) -> OmniFoldConfig:
        """Create an instance of OmniFoldConfig from a yaml file at the given path."""
    ...
    def create_instance(self) -> OmniFold:
        """Create an instance of OmniFold from the configuration."""
        omnifold = OmniFold(**self.model_dump())
        return omnifold
    ...
    def create_instance_from_yaml_file(
        ...
        """Create an instance of OmniFold from a yaml file at the given path."""
        omnifold_config = cls.from_yaml_file(path, dotpath)
        return omnifold_config.create_instance()
...
class TrainerConfig(BaseModelWithExtra):
    ...
    model: OmniFoldConfig | None = None
    num_train_steps: int
    ...
    def create_instance(
        ...
        omnifold.diffusion_num_augmentations = trainer_kwargs.pop("diffusion_num_augmentations")
        omnifold.diffusion_add_smooth_lddt_loss = trainer_kwargs.pop(
            "diffusion_add_smooth_lddt_loss"
        )
        omnifold.diffusion_add_bond_loss = trainer_kwargs.pop("diffusion_add_bond_loss")
        omnifold.train_structure_and_distogram = trainer_kwargs.pop(
            "train_structure_and_distogram"
        )
        omnifold.train_pae = trainer_kwargs.pop("train_pae")
...
create_omnifold_from_yaml = OmniFoldConfig.create_instance_from_yaml_file
