...
from omnifold.common.biomolecule import (
    Biomolecule,
    _from_mmcif_object,
...
from omnifold.data import mmcif_parsing, msa_pairing, msa_parsing, template_parsing
from omnifold.data.data_pipeline import (
    FeatureDict,
    make_msa_features,
    make_msa_mask,
...
from omnifold.data.weighted_pdb_sampler import WeightedPDBSampler
from omnifold.life import (
    ATOM_BONDS,
    ATOMS,
    DNA_NUCLEOTIDES,
    HUMAN_AMINO_ACIDS,
...
from omnifold.tensor_typing import Bool, Float, Int, typecheck
...
from omnifold.utils.utils import default, exists, first, maybe_cache, not_exists
...
def compose(*fns: Callable):
    """Chain e.g., from OmniFoldInput -> MoleculeInput -> AtomInput."""
...
# atom level, what OmniFold accepts
...
def atom_ref_pos_to_atompair_inputs(
    ...
    # concat all into atompair_inputs for projection into atompair_feats within OmniFold
...
# `n` here refers to the actual number of molecules, NOT the `n` used within OmniFold
...
def molecule_lengthed_molecule_input_to_atom_input(
    ...
    # 2. Bool['n num_mods'], where it will be passed to OmniFold for molecule modification embeds
...
# omnifold input - support polypeptides, nucleic acids, metal ions + any number of ligands + misc biomolecules
...
@typecheck
def omnifold_input_to_molecule_lengthed_molecule_input(
    omnifold_input: OmniFoldInput,
) -> MoleculeLengthMoleculeInput:
    """Convert an OmniFoldInput to a MoleculeLengthMoleculeInput."""
    i = omnifold_input
    ...
    if exists(atom_pos):
        ...
        for mol_ligand in mol_ligands:
            ...
            # NOTE: since `OmniFoldInput` is only used for inference, we can safely assume that
    ...
    assert num_tokens > 0, "You have requested an empty OmniFoldInput."
...
@typecheck
def omnifold_inputs_to_batched_atom_input(
    inp: OmniFoldInput | PDBInput | List[OmniFoldInput | PDBInput], **collate_kwargs
) -> BatchedAtomInput:
    """Convert a list of OmniFoldInput or PDBInput objects into a BatchedAtomInput object."""
    if isinstance(inp, OmniFoldInput):
        inp = [inp]
...
@typecheck
def omnifold_input_to_pdb_input(
    omnifold_input: OmniFoldInput,
    ...
    """Convert an OmniFoldInput object into a PDBInput object.

    :param omnifold_input: The OmniFoldInput object for multi-domain biomolecules.
    ...
    """
    # Get a `Biomolecule` representation of the `OmniFoldInput` object
    biomol = omnifold_input_to_biomolecule(omnifold_input, atom_positions=atom_positions)

    # Craft a `PDBInput` object using the new `Biomolecule` representation
    stub_mmcif_filepath = f"{omnifold_input.input_id}.cif"
    ...
    pdb_input = PDBInput(
        ...
        example_source=omnifold_input.example_source,
    )
...
@typecheck
def omnifold_input_to_biomolecule(
    omnifold_input: OmniFoldInput, atom_positions: np.ndarray | None = None
) -> Biomolecule:
    """Convert an OmniFoldInput object into a Biomolecule object.
    ...
    """
    input_transform = INPUT_TO_ATOM_TRANSFORM[OmniFoldInput]
    omnifold_atom_input = input_transform(omnifold_input)

    # Ensure that the atom positions are of the correct shape
    if exists(atom_positions):
        assert atom_positions.shape[0] == len(
            omnifold_atom_input.molecule_ids
    ...
    molecule_ids = omnifold_atom_input.molecule_ids.cpu().numpy()
    molecule_types = np.nonzero(omnifold_atom_input.is_molecule_types)[:, 1]

    is_molecule_mod = omnifold_atom_input.is_molecule_mod.cpu().numpy()
    molecule_atom_indices = omnifold_atom_input.molecule_atom_indices.cpu().numpy()

    atom_ids = omnifold_atom_input.atom_ids.cpu().numpy()
    ...
    residue_index = omnifold_atom_input.additional_molecule_feats.cpu().numpy()[:, 0] + 1

    # Step 6. Chain Index
    # chain_index: np.array [num_res], borrow the entity IDs (vs sym_ids, idx3) as chain IDs
    chain_index = omnifold_atom_input.additional_molecule_feats.cpu().numpy()[:, 2]
    ...
    @typecheck
    def get_sequences(i: OmniFoldInput, chain_index: Int[" n"], residue_index: Int[" n"], one_token_per_atom: Bool[" n"], is_molecule_mod: Bool[" n"]) -> List[str]:  # type: ignore
    ...
    sequences = get_sequences(
        omnifold_input, chain_index, residue_index, one_token_per_atom_mask, is_molecule_mod
    )
    ...
    chemtypes = np.nonzero(omnifold_atom_input.is_molecule_types.cpu().numpy())[1]
    ...
    if not exists(atom_positions):
        atom_positions_[atom_masks.astype(bool)] = omnifold_atom_input.atom_pos.cpu().numpy()
...
def collate_inputs_to_batched_atom_input(
    ...
    for key, grouped in zip(keys, zip(*atom_inputs)):
        ...
        # use -1 for padding int values, for assuming int are labels - if not, handle within omnifold
...
INPUT_TO_ATOM_TRANSFORM = {
    ...
    OmniFoldInput: compose(
        omnifold_input_to_molecule_lengthed_molecule_input,
