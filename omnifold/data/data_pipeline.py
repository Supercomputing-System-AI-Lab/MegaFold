...
from beartype.typing import Any, Dict, List, MutableMapping, Tuple
...
from omnifold.data import mmcif_parsing, msa_pairing, msa_parsing
from omnifold.data.kalign import _realign_pdb_template_to_query
from omnifold.data.template_parsing import TEMPLATE_TYPE, _extract_template_features
from omnifold.tensor_typing import typecheck
from omnifold.utils.data_utils import make_one_hot_np
...
FeatureDict = MutableMapping[str, list | np.ndarray | Tensor | Any]
...
@typecheck
def convert_numpy_chains_to_tensors(
    chains: Dict[str, np.ndarray], max_msas_per_chain: int | None = None
) -> FeatureDict:
    ...
    num_msa = len(chains["msa_all_seq"])
    ...
    for key, value in chains.items():
        ...
        if isinstance(value, np.ndarray) and value.dtype != np.object_:
            ...
            if exists(max_msas_per_chain) and len(tensor_chains[base_key]) == num_msa:
                tensor_chains[base_key] = tensor_chains[base_key][:max_msas_per_chain]
...
def merge_chain_features(
    ...
    # NOTE: Paired (unfiltered) and unpaired (filtered) MSA features
    # will simply be concatenated, similar to Section 2.3 of the
    # AlphaFold 3 supplement. However, unpaired (unfiltered) features
    # will also be concatenated after the unpaired (filtered) features.
    ...
    chains_merged = convert_numpy_chains_to_tensors(
...
def make_msa_features(
    ...
    :param inference: Whether to curate MSA features for inference.
    ...
    for chain_id, msa in msas.items():
        ...
        for sequence_index, sequence in enumerate(msa.sequences):
            if sequence_index == 0:
                if sequence not in entity_ids:
                    entity_ids[sequence] = len(entity_ids)
            ...
            for idx, (chemtype, residue_index) in enumerate(
                ...
                if chemtype not in chemtype_constants_cache:
                    chemtype_constants_cache[chemtype] = get_residue_constants(
                        res_chem_index=chemtype
                    )
            ...
            is_unfiltered_sequence = msa.is_unfiltered[sequence_index]
            is_unfiltered_list.append(is_unfiltered_sequence)

            # Parse species ID for unfiltered MSA pairing if possible.
            species_id = ""
            if is_unfiltered_sequence:
        ...
        padding_is_unfiltered = is_unfiltered * num_padding_alignments
        ...
        chain["is_unfiltered"] = np.array(is_unfiltered_list + padding_is_unfiltered, dtype=bool)
    ...
    for chain_index, (chain_id, template) in enumerate(templates.items()):
        ...
        for template_index, (template_biomol, template_type) in enumerate(template):
            try:
                ...
                template_chem_comp_table = {
                    comp.id: comp for comp in template_biomol.chem_comp_table
                }
                template_res_chem_comp_details = [
                    template_chem_comp_table[chemid] for chemid in template_biomol.chemid
                ]
                ...
                template_features = _extract_template_features(
                    ...
                    template_res_chem_comp_details=template_res_chem_comp_details,
                    query_sequence=query_sequence,
                    query_chemtype=chain_chemtype,
                    num_restype_classes=num_restype_classes,
                    num_distogram_bins=num_distogram_bins,
                    verbose=False,  # NOTE: Too verbose in this context.
                )
...
@typecheck
def get_assembly(biomol: Biomolecule, assembly_id: str | None = None) -> Biomolecule:
