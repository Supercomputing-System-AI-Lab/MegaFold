...
def fold(entities, request):
    ...
    model.eval()
    (structure,) = model.forward_with_megafold_inputs(
        megafold_inputs=megafold_input,
...
def start_gradio_app():
    ...
    with gr.Blocks(delete_cache=(600, 3600)) as gradio_app:
        ...
        with gr.Row():
            gr.Markdown("### MegaFold PyTorch Web UI")
...
@click.option(
    ...
    "--checkpoint",
    type=str,
    help="Path to MegaFold checkpoint",
    required=True,
)
@click.option(
    "-cache",
...
def app(checkpoint: str, cache_dir: str, precision: str):
    """Run the MegaFold PyTorch web UI."""
    ...
    global model
    model = MegaFold.init_and_load(str(path))
