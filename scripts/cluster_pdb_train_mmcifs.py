...
import polars as pl
import rootutils
from beartype.typing import Dict, List, Literal, Set, Tuple, Union
        ...
        for res_index, res in enumerate(chain):
            ...
            for atom in res:
                for neighbor in neighbor_search.search(
                    ...
                    molecule_index_suffix = (
                        f"-{res_id}" if clustering_molecule_type == "ligand" else ""
                    )
                    neighbor_molecule_index_suffix = (
                        f"-{neighbor_res_id}"
                    ...
                    atom_interface_key = (
                        f"{chain.id}:{clustering_molecule_type}{molecule_index_suffix}"
                    )
                    neighbor_interface_key = f"{neighbor_chain_id}:{neighbor_clustering_molecule_type}{neighbor_molecule_index_suffix}"
            ...
            for structure_id, chain_sequences in structure_chain_sequences.items():
                for chain_id, sequence in chain_sequences.items():
                    ...
                    if mol_type == molecule_type:
                        ...
                        molecule_id = f"{structure_id}{chain_id_}:{molecule_type_and_name[0]}{molecule_index_suffix}"
        ...
        for structure_id, chain_sequences in structure_chain_sequences.items():
            for chain_id, sequence in chain_sequences.items():
                ...
                if molecule_type_and_name[0] == "ligand":
                    ...
                    chain_cluster_mapping[
                        f"{structure_id}{chain_id_}:{molecule_type_and_name[0]}{molecule_index_suffix}"
                    ] = cluster_id
